...
<html>
    ...
    <body>
        ...
        @endphp
        <form action="{{ route('inventories.update', $firstItem->id) }}" method="POST">
        @csrf
        @method('PUT')

      
        <div>
            <label for="item_name">Inventory Name</label>
            <input type="text" name="item_name" id="name" value="{{ old('item_name', $firstItem->item_name) }}" required>
        </div>

        <div>
            <label for="item_description">Description</label>
            <textarea name="item_description" id="content" required>{{ old('item_description', $firstItem->item_description) }}</textarea>
        </div>

        <div>
            <label for="quantity_per_unit">Pieces per Unit:</label>
            <input required type="number" id="unit_quantity" name="quantity_per_unit" min='1' value={{ old('quantity_per_unit', $firstItem->quantity_per_unit) }}>
        </div>

        <button type="submit">Update Item</button>
        <a href="{{ route('viewInventories') }}">Cancel</a>
    </form>
        @if ($errors->any())
        <div>
            <ul>
